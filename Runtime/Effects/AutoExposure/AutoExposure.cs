...
namespace NaninovelPostProcess
{
	...
	public class AutoExposure : PostProcessSpawnObject, Spawn.IParameterized, Spawn.IAwaitable, DestroySpawned.IParameterized, DestroySpawned.IAwaitable
	{
		...
		{
			if (duration > 0) await filteringTweener.RunAsync(new VectorTween(autoExposure.filtering.value, filtering, duration, x => autoExposure.filtering.value = x, IgnoreTimescale), asyncToken, autoExposure);
		...
		{
			if (duration > 0) await minimumTweener.RunAsync(new FloatTween(autoExposure.minLuminance.value, minimum, duration, x => autoExposure.minLuminance.value = x, IgnoreTimescale), asyncToken, autoExposure);
		...
		{
			if (duration > 0) await maximumTweener.RunAsync(new FloatTween(autoExposure.maxLuminance.value, maximum, duration, x => autoExposure.maxLuminance.value = x, IgnoreTimescale), asyncToken, autoExposure);
		...
		{
			if (duration > 0) await exposureCompensationTweener.RunAsync(new FloatTween(autoExposure.keyValue.value, exposureCompensation, duration, x => autoExposure.keyValue.value = x, IgnoreTimescale), asyncToken, autoExposure);
		...
		{
			if (duration > 0) await speedUpTweener.RunAsync(new FloatTween(autoExposure.speedUp.value, speedUp, duration, x => autoExposure.speedUp.value = x, IgnoreTimescale), asyncToken, autoExposure);
		...
		{
			if (duration > 0) await speedDownTweener.RunAsync(new FloatTween(autoExposure.speedDown.value, anamorphicRatio, duration, x => autoExposure.speedDown.value = x, IgnoreTimescale), asyncToken, autoExposure);
