...
namespace NaninovelPostProcess { 
	...
	public class Vignette : PostProcessSpawnObject, Spawn.IParameterized, Spawn.IAwaitable, DestroySpawned.IParameterized, DestroySpawned.IAwaitable, PostProcessSpawnObject.ITextureParameterized
	{
		...
		{
			if (duration > 0) await colorTweener.RunAsync(new ColorTween(vignette.color.value, color, ColorTweenMode.All, duration, x => vignette.color.value = x, IgnoreTimescale), asyncToken, vignette);
		...
		{
			if (duration > 0) await centerTweener.RunAsync(new VectorTween(vignette.center.value, center, duration, x => vignette.center.value = x, IgnoreTimescale), asyncToken, vignette);
		...
		{
			if (duration > 0) await intensityTweener.RunAsync(new FloatTween(vignette.intensity.value, intensity, duration, x => vignette.intensity.value = x, IgnoreTimescale), asyncToken, vignette);
		...
		{
			if (duration > 0) await smoothnessTweener.RunAsync(new FloatTween(vignette.smoothness.value, smoothness, duration, x => vignette.smoothness.value = x, IgnoreTimescale), asyncToken, vignette);
		...
		{
			if (duration > 0) await roundnessTweener.RunAsync(new FloatTween(vignette.roundness.value, roundness, duration, x => vignette.roundness.value = x, IgnoreTimescale), asyncToken, vignette);
		...
		{
			if (duration > 0) await opacityTweener.RunAsync(new FloatTween(vignette.opacity.value, opacity, duration, x => vignette.opacity.value = x, IgnoreTimescale), asyncToken, vignette);
