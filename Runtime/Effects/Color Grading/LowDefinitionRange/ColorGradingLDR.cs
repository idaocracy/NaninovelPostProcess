...
namespace NaninovelPostProcess { 
    ...
    public class ColorGradingLDR : PostProcessSpawnObject, Spawn.IParameterized, Spawn.IAwaitable, DestroySpawned.IParameterized, DestroySpawned.IAwaitable, PostProcessSpawnObject.ITextureParameterized
    {
        ...
        {
            if (duration > 0) await contributionTweener.RunAwaitable(new FloatTween(colorGrading.ldrLutContribution.value, contribution, new(duration), x => colorGrading.ldrLutContribution.value = x/*, IgnoreTimescale*/), asyncToken, colorGrading);
        ...
        {
            if (duration > 0) await brightnessTweener.RunAwaitable(new FloatTween(colorGrading.brightness.value, brightness, new(duration), x => colorGrading.brightness.value = x/*, IgnoreTimescale*/), asyncToken, colorGrading);
        ...
        public override List<ICommandParameterData> GetParams()
        {
            return new List<ICommandParameterData>()
            {
                ...
                { new CommandParameterData<Color>("ColorFilter", () => colorGrading.colorFilter.value, v => colorGrading.colorFilter.value = v, (i,p) => i.ColorField(p, includeAlpha:false, includeHDR:true), defaultColorFilter)},
